...
export default function AssignmentDetail() {
  ...
  if (!assignment) {
    return (
      <main className="min-h-screen bg-background flex items-center justify-center font-heading text-primary">
  ...
  return (
    <main className="max-w-[1920px] w-full mx-auto relative bg-background overflow-clip font-heading text-primary">
        ...
        <div className="rounded-[20px] bg-secondary p-8 md:p-[clamp(24px,4vw,80px)]">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <a
              ...
              className="flex flex-row items-center gap-2 py-3 px-6 bg-[#c96b8a] hover:bg-[#b45b78] rounded-[12px] hover:scale-[1.02] transition-transform text-white font-bold text-[clamp(14px,1.2vw,18px)] whitespace-nowrap shrink-0"
            >
      ...
      >
        <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#3d3a37] p-8 md:p-12 bg-[#fdf0f3]">
      ...
      <section className="mt-[clamp(30px,5vw,80px)] mx-4 md:mx-10 flex flex-col gap-[clamp(24px,4vw,60px)] relative z-10 pb-[clamp(60px,10vw,150px)]">
        {assignment.sections.map((section, si) => (
          ...
          >
            ...
            {/* Content */}
            {section.content && (
              <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#3d3a37] p-8 bg-[#fdf0f3]">
            ...
            {section.steps && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.steps.map((s, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-6 rounded-[16px] shadow-[inset_0_0_0_1px_#3d3a37] bg-[#fdf0f3] hover:bg-[#f8e1e7] transition-all"
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-[clamp(13px,1vw,16px)] font-light leading-[1.6] text-primary/70">{s.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {section.highlights && (
              <div className="flex flex-col gap-3">
                {section.highlights.map((h, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-5 rounded-[14px] bg-[#fdf0f3] shadow-[inset_0_0_0_1px_#3d3a37]"
                  >
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-8 border-t border-primary/20"
        >
          <Link
            to="/#project"
            className="flex flex-row items-center gap-2 py-3 px-6 rounded-[57px] shadow-[inset_0_0_0_1px_#3d3a37] hover:bg-[#fdf0f3] transition-all font-bold text-[clamp(14px,1.2vw,18px)]"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex flex-row items-center gap-2 py-3 px-6 bg-[#c96b8a] hover:bg-[#b45b78] rounded-[12px] hover:scale-[1.02] transition-transform text-white font-bold text-[clamp(14px,1.2vw,18px)]"
          >
            <ExternalLink className="w-4 h-4" />
            Xem báo cáo (PDF)
          </a>
